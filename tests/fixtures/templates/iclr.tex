...
We find multimodal loss follows a modified power law: $L(N_V, N_L, D) = ((N_{V,c}) / N_V)^{\alpha_V} + ((N_{L,c}) / N_L)^{\alpha_L} + ((D_c) / D)^{\alpha_D} + L_infinity$
